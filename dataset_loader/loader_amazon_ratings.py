...
def load_amazon_ratings(npz_path, config, split_idx=0, ood_classes=(0,1), id_classes=(2,3,4)):
    """
    amazon_ratings.npz -> PyG loaders for transductive OOD:
      - train: ID only
      - val:   ID + OOD
      - test:  ID + OOD
    Masks in the NPZ are shaped (splits, nodes). Edges are (E,2).
    """
    z = np.load(npz_path + 'amazon_ratings/amazon_ratings.npz', allow_pickle=True)
    ...
    # Sanity checks
    N = x_np.shape[0]
    assert y_np.shape[0] == N, "labels length != num nodes"
    assert edges_np.ndim == 2 and edges_np.shape[1] == 2, "edges must be (E,2)"
    assert train_masks.ndim == 2 and train_masks.shape[1] == N, "train_masks must be (S,N)"
    assert val_masks.shape[1] == N and test_masks.shape[1] == N, "val/test masks must be (S,N)"
    assert 0 <= split_idx < train_masks.shape[0], "split_idx out of range"

    # PyG Data expects edge_index shape (2, E)
    edge_index = torch.from_numpy(edges_np.T).long()  # transpose (E,2)->(2,E)
    ...
    # --- 2) Choose split row -> (N,) boolean masks
    ...
    # --- 3) OOD policy (train = ID only; val/test = all) ---
    ...
    # --- 4) Overwrite labels as one-hot over ID classes; OOD rows remain zeros ---
    num_id = len(id_classes)
    new_y = torch.zeros((N, num_id), dtype=torch.float)
    id_min = min(id_classes)
    id_labels = (data.y[id_node_mask] - id_min).clamp(min=0)  # {2,3,4}->{0,1,2}
    new_y[id_node_mask] = F.one_hot(id_labels, num_classes=num_id).float()
    data.y = new_y
    ...
    print("--- Amazon-Ratings (ID train / ID+OOD val,test) ---")
